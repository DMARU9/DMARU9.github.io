...
VS Code のエージェントセレクターから対象のエージェントを選択し、チャットで指示します。
...
各コンパイラには Python スクリプトが附属しており、以下のような役割分担になっています。
...
コンパイルが完了した日報には `compiled: true` が設定されます。次回オーケストレーターを実行したとき、このフラグがついたファイルは自動的にスキップされるため、同じ内容が二重に処理されることはありません。
